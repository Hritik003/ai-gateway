...
6. [The Core Problem: Multiplexing Without Sessions](#the-core-problem-multiplexing-without-sessions)
7. [Compatibility Matrix](#compatibility-matrix)
   - 7.1 [Cell 1 — Legacy Client ↔ Legacy Backend](#cell-1--legacy-client--legacy-backend)
   - 7.2 [Cell 2 — Modern Client ↔ Modern Backend](#cell-2--modern-client--modern-backend)
   - 7.3 [Cell 3 — Modern Client ↔ Legacy Backend](#cell-3--modern-client--legacy-backend)
   - 7.4 [Cell 4 — Legacy Client ↔ Modern Backend](#cell-4--legacy-client--modern-backend)
8. [Design Decisions](#design-decisions)
   - 8.1 [D1 — Downstream (Client) Era Detection](#d1--downstream-client-era-detection)
   - 8.2 [D2 — Upstream (Backend) Era Detection](#d2--upstream-backend-era-detection)
   - 8.3 [D3 — Capability / server/discover Cache](#d3--capability--serverdiscover-cache)
   - 8.4 [D4 — MRTR Translation Strategy](#d4--mrtr-translation-strategy)
   - 8.5 [D5 — Upstream Session Pool (Cell 3)](#d5--upstream-session-pool-cell-3)
9. [Phased Implementation Plan](#phased-implementation-plan)
   - 9.1 [Phase 0 — Foundations (No Behavior Change)](#phase-0--foundations-no-behavior-change)
   - 9.2 [Phase 1 — Modern ↔ Modern (Stateless) + Legacy ↔ Legacy (Unchanged)](#phase-1--modern--modern-stateless--legacy--legacy-unchanged)
   - 9.3 [Phase 2 — Cross-Era Translation (Cells 3 and 4)](#phase-2--cross-era-translation-cells-3-and-4)
   - 9.4 [Phase 3 — Cacheable Results, Deterministic Ordering, Minor Spec Items](#phase-3--cacheable-results-deterministic-ordering-minor-spec-items)
   - 9.5 [Phase 4 — Auth Hardening, Deprecations, and Cleanup](#phase-4--auth-hardening-deprecations-and-cleanup)
10. [Spec Item → Phase Mapping](#spec-item--phase-mapping)
11. [Gotchas and Risks](#gotchas-and-risks)
12. [Testing and Verification](#testing-and-verification)
13. [go-sdk Dependency](#go-sdk-dependency)
...
| P1.2 | **Header/`_meta` validation** (modern path) — require `Mcp-Method` == `method`; require `Mcp-Name` == `params.name\|uri` for `tools/call`, `resources/read`, `prompts/get` (decode base64 sentinel); require three `_meta` fields; `MCP-Protocol-Version` == `_meta` version and in `supportedVersions`. Emit `-32020`/`-32602`/`-32022` | #A4, #D1 | `handlers.go` |
...
Modern routing relies on `x-ai-eg-mcp-route` being present on **all** methods, not just `initialize`. Verify the Envoy `HTTPRouteFilter` sets it unconditionally. Today only `initialize` reads it (`handlers.go:376`).
...
Confirm `github.com/modelcontextprotocol/go-sdk` (pinned in `go.mod`) exposes `2026-07-28` types: `DiscoverResult`, `InputRequiredResult`, `resultType`, `server/discover`, `subscriptions/listen`. If not, bump it or hand-roll the message shapes. **This must be verified before starting Phase 1.**

### Deprecated but Live

Roots, Sampling, and Logging must keep working during the 12-month deprecation window. Do not strip handlers; add deprecation warnings.

### Auth Session Coupling

The stateless path must enforce per-request bearer auth. Verify no session-coupled authorization logic exists. `authorization.go` is already per-request — confirm no hidden session dependency.
...
Each gateway instance maintains its own upstream session pool for legacy backends. This means the same backend may have multiple concurrent sessions from different gateway instances. This is correct but wasteful. For most deployments where the number of legacy backends is small, this is acceptable. Document this as a known limitation.

## Testing and Verification

### Unit Tests

Mirror existing style in `internal/mcpproxy/*_test.go`:

- Modern `tools/list` fan-out + merge from capability cache.
- Modern `tools/call` routed by `backend__tool` prefix, header/body recomputation verified.
- Missing `_meta` ⇒ `-32602`/`400`.
- Header mismatch ⇒ `-32020`/`400`.
- Unsupported version ⇒ `-32022`/`400` (response includes `supportedVersions`).
- `server/discover` merge across multiple backends.
- `subscriptions/listen` merged stream + keep-alive comment lines.
- Cache invalidation on `*_list_changed` notifications.
- GET on modern endpoint ⇒ `405`.
- DELETE on modern endpoint ⇒ `405`.
- `ping` on modern endpoint ⇒ `-32601`.

### Matrix Tests (Phase 2)

Table-driven `client-era × backend-era` tests against fake legacy and modern backends:

- MRTR↔SSE translation round-trip fidelity.
- Upstream session pool creation and teardown.
- `server/discover` shim for legacy backends.
- Error code translation (`-32002` ↔ `-32602`).

### Regression

Run the existing MCP proxy test suite unchanged to prove **Cell 1 (legacy↔legacy)** is untouched after each phase.

### End-to-End

One MCPRoute fronting a modern stateless backend and a legacy session backend. Exercise with:

1. A modern client — raw `curl` POSTs carrying `_meta` + `Mcp-Method`/`Mcp-Name`.
2. An existing legacy client/SDK.

Confirm both work and `server/discover` returns merged capabilities from both backends.

## go-sdk Dependency

Before starting Phase 1, verify the Go SDK exposes the following types and constants for the `2026-07-28` revision:

| Type/Constant | Used For |
|--------------|----------|
| `DiscoverResult` | `server/discover` response shape |
| `DiscoverParams` | `server/discover` request params |
| `InputRequiredResult` | MRTR interim result |
| `resultType` field on all `Result` types | Required by spec |
| `SubscriptionsListenParams` | `subscriptions/listen` request |
| Protocol version constant `2026-07-28` | Version negotiation |
| Error codes `-32020`, `-32021`, `-32022` | Modern error handling |

If the SDK does not yet expose these, either:
- Bump to a version that does.
- Hand-roll the message shapes as local types and migrate when the SDK catches up.
